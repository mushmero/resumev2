...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <form class="form" action="{{ route('statuslist.update',$data->id) }}" method="post">
                @method('put')
                @csrf
                <x-adminlte-card title="{{ $table_title }} {{ $data->status }}" theme="default" icon="fas fa-sm fa-wrench" collapsible>
                    <x-adminlte-input name="status" label="Status" fgroup-class="col-md-6 row" label-class="col-md-2 control-label" igroup-class="col-md-10" value="{{ $data->status }}" enable-old-support/>
