...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <form class="form" action="{{ route('educationlevel.update',$data->id) }}" method="post">
                @method('put')
                @csrf
                <x-adminlte-card title="{{ $table_title }} {{ $data->name }}" theme="default" icon="fas fa-sm fa-wrench" collapsible>
                    ...
                    <x-adminlte-input name="level" label="Level" fgroup-class="col-md-6 row" label-class="col-md-2 control-label" igroup-class="col-md-10" value="{{ $data->level }}" enable-old-support/>
